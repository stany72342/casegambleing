...
export const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-0 md:p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-7xl bg-slate-950 border-0 md:border-2 border-red-900 md:rounded-2xl overflow-hidden shadow-[0_0_100px_rgba(220,38,38,0.2)] h-full md:h-[90vh] flex flex-col md:flex-row">
        
        {/* Mobile Header (Only visible on small screens) */}
        <div className="md:hidden p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900">
             <h2 className="text-xl font-black text-red-500 flex items-center gap-2"><ShieldAlert /> ADMIN</h2>
             <div className="flex gap-2">
                 <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 bg-slate-800 text-white rounded">
                     {isSidebarOpen ? <X size={20} /> : <Menu size={20} />}
                 </button>
                 <button onClick={props.onClose} className="p-2 bg-slate-800 text-red-400 rounded"><LogOut size={20}/></button>
             </div>
        </div>

        {/* GUI Organizer Sidebar */}
        <div className={`
            md:w-64 bg-slate-900 border-r border-slate-800 flex flex-col
            ${isSidebarOpen ? 'block absolute inset-0 z-50' : 'hidden md:flex'}
        `}>
            {/* Desktop Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-950">
            <h2 className="text-xl md:text-3xl font-black text-white mb-4 md:mb-8 border-b border-slate-800 pb-4 uppercase flex justify-between items-center">
                {CATEGORIES.flatMap(c => c.items).find(t => t.id === activeTab)?.label}
                <span className="text-[10px] md:text-xs font-mono text-slate-600 hidden md:inline">SESSION: {Date.now()}</span>
            </h2>
