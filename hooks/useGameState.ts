...
import { GameState, Item, ItemTemplate, Rarity, RARITY_ORDER, AuctionListing, TradeOffer, PromoCode, LogEntry, Role, Case, GameConfig, ScheduledEvent, UserAccount, Announcement, ShopEntry, ActiveGiveaway, InboxMessage, GameUpdate, GameSettings, ChatMessage, UserReport } from '../types';
import { DEFAULT_ITEMS, DEFAULT_CASES, INITIAL_STATE, XP_PER_LEVEL_BASE, XP_MULTIPLIER, BAD_WORDS } from '../constants';

const STORAGE_KEY = 'case_clicker_v2_7_god_mode'; 
...
export const useGameState = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Deep merge config to ensure new toggles exist
        const newConfig = { 
            ...INITIAL_STATE.config, 
            ...parsed.config, 
            gameSettings: { ...INITIAL_STATE.config.gameSettings, ...(parsed.config?.gameSettings || {}) },
            featureToggles: { ...INITIAL_STATE.config.featureToggles, ...(parsed.config?.featureToggles || {}) } 
        };
        
        // Merge Items and Cases to ensure new content exists in old saves
        const mergedItems = { ...DEFAULT_ITEMS, ...parsed.items };
        const mergedCases = [...DEFAULT_CASES]; 
        const finalCases = parsed.cases && parsed.cases.length > 0 ? parsed.cases : mergedCases;
        ...
        setGameState({ 
            ...
            inbox: parsed.inbox || [],
            updates: parsed.updates || INITIAL_STATE.updates,
            reports: parsed.reports || INITIAL_STATE.reports,
  ...
  // 2. Scheduler, Bots & Security Check
  useEffect(() => {
      if (!loaded) return;
      const interval = setInterval(() => {
          const now = Date.now();
      ...
      setGameState(prev => {
          ...
          if (prev.username && updatedUserDB[prev.username]) {
              updatedUserDB[prev.username] = {
                  ...
                  // NOTE: Syncing full inventory to DB in real app is heavy, here we just do count for leaderboard
                  stats: {
              ...
              // For simulation purposes, we also update the 'offline' inventory if we want admin to see it,
              // but since 'inventory' is separate in state, we leave it. Admin inspects `gameState.inventory` if it's the logged in user.
              // To enable Admin inspection of others, we'd need to store full inventory in `userDatabase`.
              // Let's add that logic for the "Inspector" feature.
  ...
  const adminSendMail = useCallback((to: string | 'ALL', subject: string, body: string) => {
      setGameState(prev => {
          ...
          if (to === 'ALL') {
              ...
              // Update current user inbox if they are logged in
              ...
              // Update current user inbox if they are the recipient
  ...
  const adminRemoveItemFromUser = useCallback((targetUser: string, itemId: string) => {
      setGameState(prev => {
          ...
          // If targeting self (rare but possible), update live inventory
  ...
  return {
    ...
    sendAdminEmail: () => {}, createPromoCode, redeemPromoCode,
    ...
    // SAVE EXPORTS
    importSave: (json: string) => { try { setGameState(JSON.parse(json)); return true; } catch { return false; } },
