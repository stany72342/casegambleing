...
const App = () => {
  ...
  if (gameState.config.maintenanceMode && !gameState.isAdmin) {
      return (
  ...
  const isTabEnabled = (tab: string) => {
      ...
      // Casino sub-games are technically enabled if specific toggle is on, 
      // but they are now accessed via the main 'casino' hub which doesn't have a single toggle.
      // We assume if at least one game is on, casino is valid. 
      // But for specific routes:
      ...
      if (tab === 'auction' && !toggles.auction) return false;
      if (tab === 'trade' && !toggles.trading) return false;
      return true;
  };
  ...
  return (
    <div className={`min-h-screen text-white font-sans pb-20 relative transition-colors duration-500 ${getThemeClasses()}`}>
      ...
      {/* MOTD Banner */}
      {gameState.motd && (
          <div className="bg-purple-900/90 text-white font-bold text-center py-2 px-4 shadow-lg flex items-center justify-center gap-2 relative z-40 border-b border-purple-500">
              <Icons.Info size={16} />
              {gameState.motd}
          </div>
      )}
      ...
      <main className="container mx-auto px-4 mt-8 animate-in fade-in duration-500">
        ...
        {currentTab === 'trade' && (
            <Trade
                gameState={gameState}
                createTradeListing={createTradeListing}
                fulfillTrade={fulfillTrade}
